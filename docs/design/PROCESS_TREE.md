...
Tree mode preserves the current columns, row height, independent process counters, resizing/reordering, scrollbars, inspector and process actions. Only the name cell gains indentation and disclosure controls. Names stay clipped inside their own column; indentation adapts to its width. A tooltip and accessibility description identify the immediate parent and the actual nesting level even in deeply nested trees. Metrics remain per-process rather than summing children into a parent.
...
| Sorting | Sort roots and siblings by the chosen column, keeping descendants with their parent. |
...
| Copy/export | Copy selected rows in displayed order. Toolbar exports follow the visible tree order; process records retain their real parent PID. |
